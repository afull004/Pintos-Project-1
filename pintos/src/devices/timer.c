...
// List of all sleeping threads in wait state
...
timer_interrupt (struct intr_frame *args UNUSED)
{
	...
	struct list_elem *topElem = list_begin(&sleepingThreads);
	struct list_elem *bottomElem = list_end(&sleepingThreads);
	while(!list_empty(&sleepingThreads)){//topElem != bottomElem){
		struct thread *t = list_entry(topElem, struct thread, elem);
		if(t->endTime <= timer_ticks()){
			list_remove(topElem);
			thread_unblock(t);
			topElem = list_begin(&sleepingThreads);
		}else{
			break;
		}
	}
